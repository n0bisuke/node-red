...
function ArduinoNode(n) {
    ...
    console.log("[firmata] Opening "+this.device);
    var node = this;
        ...
        if (!arduinoReady) {
            if (thisboard == null) {
                node.board = new firmata.Board(node.device, function(err) {
                    ...
                    console.log('[firmata] Arduino connected');
                });
            }
            else {
                ...
                clearInterval(node.toun);
                node.toun = false;
                console.log("[firmata] Arduino already connected");
            }
        } else { console.log("[firmata] Waiting for Firmata"); }
    ...
    this.on('close', function() {
        ...
        if (node.toun) {
            clearInterval(node.toun);
            console.log("[firmata] arduino wait loop stopped");
        }
        console.log("[firmata] Stopped");
    });
}
...
function DuinoNodeIn(n) {
    ...
    if (typeof this.serverConfig === "object") {
        ...
        this.on('close', function() {
            if (node.toui) {
                clearInterval(node.toui);
                console.log("[firmata] input wait loop stopped");
            }
        });
    }
    else {
        console.log("[firmata] Serial Port not Configured");
    }
}
...
function DuinoNodeOut(n) {
    ...
    if (typeof this.serverConfig === "object") {
        ...
        node.touo = setInterval(function() {
            ...
            else { console.log("[firmata] waiting for arduino to connect"); }
        }, 5000); // loop to wait for firmata to connect to arduino

        this.on('close', function() {
            if (node.touo) {
                clearInterval(node.touo);
                console.log("[firmata] output wait loop stopped");
            }
        });
    }
    else {
