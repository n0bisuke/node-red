...
ConsoleLogHandler.on("log",function(msg) {
        console.log("["+msg.level+"] ["+msg.type+":"+(msg.name||msg.id)+"] "+msg.msg);
});
        ...
        var obj = {
            register: function(type,node) {
                ...
                if (callerFilename == null) {
                    console.log("["+type+"] unable to determine filename");
                } else {
                    ...
                    if (fs.existsSync(configFilename)) {
                        ...
                        console.log("["+type+"] missing template file: "+configFilename);
                    }
                }
            },
            get: function(type) {
...
events.on('type-registered',function(type) {
        if (missingTypes.length > 0) {
            ...
            if (i != -1) {
                ...
                console.log("[red] Missing type registered: "+type);
            }
            if (missingTypes.length == 0) {
                parseConfig();
            }
...
function stopFlows() {
    if (activeConfig&&activeConfig.length > 0) {
        console.log("[red] Stopping flows");
    }
    registry.clear();
}
...
var parseConfig = function() {
    ...
    if (missingTypes.length > 0) {
        console.log("[red] Waiting for missing types to be registered:");
        for (var i in missingTypes) {
            console.log("[red]  - "+missingTypes[i]);
        }
        return;
    }

    console.log("[red] Starting flows");
    ...
    for (var i in activeConfig) {
        ...
        if (nt) {
            try {
                ...
                console.log("[red] "+activeConfig[i].type+" : "+err);
            }
        }
        // console.log(nn);
        if (nn == null) {
            console.log("[red] unknown type: "+activeConfig[i].type);
        }
    }
